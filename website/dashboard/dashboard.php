...
<?php
$delete_img_msg = "";
if (isset($_POST["delete_acc_img"])) {
  $conn = connDb();
  $img_name = $_POST["img_name"];
  $stmt = $conn->prepare("UPDATE users SET img='' WHERE id=? ;");
  $stmt->bind_param("s", $_SESSION["userId"]);
  if (!$stmt->execute()) {
    $delete_img_msg = "<p class='error'>could't delete the image</p>";
  } else {
    if (file_exists("./profiles_pic/$img_name")) {
      unlink("./profiles_pic/$img_name");
    }
    $delete_img_msg = "<p class='success'>image deleted successfully</p>";
  }
  global $delete_img_msg;
  $conn->close();
}
?>
...
<body>
  ...
  <header>
    <form action="" method="post" id="header_user_info" enctype="multipart/form-data">
      ...
      <div id="header_user_img_container" onclick="clickOn('#user_img_input')">
        <img width="50" alt="" src=<?= get_user_img() ?> id="user_img" />
  ...
  <main>
    ...
    if ($selected_page == "settings") {
      ...
      <div class='d-grid'>
        <form action='../otp/generate.php' method='POST' class='d-flex border-l rad-20 p-20 pt-30'>
          <h3 style='letter-spacing: -0.5px'>Change OTP :</h3>
          <input type='hidden' name='id' value={$_SESSION['userId']}>
          <button class='change-otp-submit-btn submit-btn' type='submit'>
            Change
          </button>
        </form>
        <form action='' method='POST' class='d-flex border-l rad-20 p-20 pt-30'>
          <h3 style='letter-spacing: -0.5px'>Delete OTP 2fa :</h3>
          <input type='hidden' name='delete_otp_id' value={$_SESSION['userId']}>
          <button class='change-otp-submit-btn submit-btn' type='submit'>
            Delete
          </button>
        </form>
        <form action='' method='POST' class='d-flex border-l rad-20 p-20 pt-30'>
          <h3 style='letter-spacing: -0.5px'>Delete the user image :</h3>
          <input type='hidden' name='img_name' value=" . get_user_img() . " >
          <button name='delete_acc_img' class='change-otp-submit-btn submit-btn' type='submit'>
            Delete
          </button>
        </form>
          ");
